...
class BaseTransferLearningLogger(ABC):
    ...
    def _count_unique_inchi_keys(self, smiles) -> Tuple[List, List]:
        ...
        ######################### byzzz
        sorted_tuple_all = sorted_tuple
        list_of_labels_all = [f"Times sampled: {v[0]}" for v in sorted_tuple]
        sorted_mols_all = [v[1] for v in sorted_tuple]
        ######################### byzzz
        ...
        ############### byzzz
        smi_file = open("/home/zhangky/projects/data/3clpro/3clpro_sampled_test.smi","w")   # change dir! 
        for idx,c in enumerate(list_of_labels_all):
            smi_file.write(Chem.MolToSmiles(sorted_mols_all[idx])+"\t"+list_of_labels_all[idx]+"\n")
        smi_file.close()
        ############### byzzz
