...
def _update_total_score(summary: ComponentSummary, query_length: int, valid_indices: List[int]) -> ComponentSummary:
    ...
    # print("len(summary.total_score):{}".format(len(summary.total_score)))
    # print("summary.total_score:{}".format(summary.total_score))
...
class BaseScoringFunction(ABC):
    ...
    def get_final_score(self, smiles: List[str]) -> FinalSummary:
        ...
        for sc in self.scoring_components:
            sc_result=sc.calculate_score(molecules)
            summaries.append(_update_total_score(sc_result,query_size, valid_indices))
