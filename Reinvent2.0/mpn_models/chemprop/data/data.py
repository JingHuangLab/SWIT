...
class MoleculeDataset(Dataset):
    ...
    def batch_graph(self) -> List[BatchMolGraph]:
        ...
        if self._batch_graph is None:
            ...
            mol_graphss = []
            for d in self._data:
                # print("d shape:{}".format(d.__sizeof__()))
                for idx,m in enumerate(d.mol):
                    try:
                        mol_graphss.append([MolGraph(m, d.atom_features)])
                    except:
                        # print("failed")
                        # print(m)
                        mol = Chem.MolFromSmiles("O=C(c1cc(Cc2n[nH]c(=O)c3ccccc23)ccc1F)N1CCN(C(=O)C2CC2)CC1")
                        mol_graphss.append([MolGraph(mol, d.atom_features)])
